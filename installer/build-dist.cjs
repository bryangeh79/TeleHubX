...
step('Copy runtime init scripts', () => {
  ...
  // vmfix20 / Issue #25: HTA loading splash spawned by telehubx-start.vbs
  ...
  // vmfix20 (Issue #28): TeleHubX Dashboard shortcut — pure browser-open,
...
step('Write VERSION.txt', () => {
  ...
  const content =
    ...
    `commit=${commit}\n` +
    `buildTime=${buildTime}\n` +
    `artifact=TeleHubX-Setup-1.0.0-vmfix20.exe\n` +
