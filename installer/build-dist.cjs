...
step('Copy runtime init scripts', () => {
  ...
  // vmfix21 / Issue #25: HTA loading splash spawned by telehubx-start.vbs
  ...
  // vmfix21 (Issue #28): TeleHubX Dashboard shortcut — pure browser-open,
...
step('Write VERSION.txt', () => {
  ...
  const content =
    ...
    `commit=${commit}\n` +
    `buildTime=${buildTime}\n` +
    `artifact=TeleHubX-Setup-1.0.0-vmfix21.exe\n` +
