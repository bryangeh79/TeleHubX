...
### C. Feature roadmap：关键词发现群强化（vmfix27+ 候选）

**痛点（2026-05-13 实测）**：dev 机用 "健康 马来西亚" 跑 `discover_groups_by_keyword` → 0 命中。
现行 `contacts.Search` 是单 API + 单关键词 + 30 条上限 + 群名/username 子串匹配，几乎所有"中文 + 地名"或"细分行业"关键词都会撞 0 命中。

**目标**：召回率 5-10×、quality 评分可执行、用户少点击。

#### 🟢 A 类：召回率（找更多群）

| # | 改进 | 工时 | ROI |
|---|---|---|---|
| **A1** | `messages.searchGlobal` 按**消息内容**搜全网公开群 | 0.5 天 | 🔥 召回 5-10× |
| **A2** | 邀请链接收割 — 进种子群扫消息正则 `t.me/+xxx`（含**私密群**）| 1 天 | 🔥 解锁私密群 |
| **A3** | **AI 关键词扩展** — 输 "健康 马来西亚" → 自动展开 5-10 个语义变体并发搜（含中英马翻译 + 同义词）| 1 天 | 🔥 同关键词命中 3-5× |
| **A4** | 跨账号 union 搜索（不同号 TG 给 personalize 不同结果）| 0.5 天 | 🟡 +30-50% |
| **A5** | `contacts.Search` 返回的 user.about 挖 channel 链接 | 1 天 | 🟡 |
| **A6** | 转发来源回溯（forwarded from 源频道）| 0.5 天 | 🟡 |
| **A7** | 跨群成员展开 `messages.GetCommonChats` | 1 天 | 🟡 |
| **A8** | tgstat.com API 接入（已索引 2M+ 群）| 1 天 + 月费 ~$50-100 | 🔥 立即上量 |

#### 🟡 B 类：评分质量

| # | 改进 | 工时 | ROI |
|---|---|---|---|
| **B1** | **quality 算法重做** — 4 维度：成员数 / 创建年龄 / 最近活跃 / 关键词相关度（25% 各）| 1 天 | 🔥 修当前 binary tendency (A 17 / C 0 / D 19) |
| **B2** | **AI 评分** — 群名 + description + 5 条近期消息 → AI 评「目标客户匹配度 0-100」| 1.5 天 | 🔥 智能筛 |
| **B3** | 敏感群过滤（赌博/色情/政治）→ 自动归 D 不入库 | 0.5 天 | 🟢 合规 |
| **B4** | 「热度趋势」标签 — 最近 7 天消息暴涨群打 🔥 | 0.5 天 | 🟡 |

#### 🟠 C 类：工作流

| # | 改进 | 工时 | ROI |
|---|---|---|---|
| **C1** | **关键词智能建议** — 输入框 AI 实时推荐近义词 | 0.5 天 | 🔥 减试错 |
| **C2** | **「发现+加群」一体化** — 预设「保守（只发现）」/「激进（A 档自动派加群）」| 1 天 | 🔥 一键 batch |
| **C3** | **失败智能 fallback** — 没匹配自动 fallback 到 searchGlobal / 同义词 / 英文翻译；UX 文案改成「试试这些关键词：xxx / yyy」+ 一键 retry 按钮 | 1 天 | 🔥 「找不到」率 30%→5% |
| **C4** | 增量发现 — 同关键词第二次跑跳过上次入库的群 | 0.3 天 | 🟢 |
| **C5** | **群预览** — 群源发现页能 peek 群（不加入，看 5 条最近消息）再决定 | 1 天 | 🔥 减盲加 |
| **C6** | A 档群批量派发加群任务（一键 batch）| 0.5 天 | 🟢 |
| **C7** | 历史命中率展示 — dashboard 列最近关键词 + 命中率 + 平均 quality | 0.5 天 | 🟡 |

#### 🔵 D 类：工程

| # | 改进 | 工时 | ROI |
|---|---|---|---|
| **D1** | 关键词 24h cache — 同 query 24h 内不重打 TG（防风控 + 省 API）| 0.5 天 | 🟢 |
| **D2** | FloodWait 自动跨账号分发（A 号限就切 B 号继续）| 1 天 | 🟢 |
| **D3** | discovered_groups `tgChatId` 唯一索引验证 query 用上 | 0.2 天 | 🟢 |
| **D4** | 任务模板系统 —「养号期发现群」/「营销前发现群」/「跨语言扩展」三套餐 | 1 天 | 🟢 |

#### ⚫ E 类：长期护城河

| # | 改进 | 工时 | ROI | 状态 |
|---|---|---|---|---|
| ~~**A8** tgstat.com API~~ | ~~第三方 200 万+ 群索引 API~~ | ~~1 天~~ | ~~🟢 立即上线大量候选~~ | ❌ **2026-05-14 排除** — 月费 $49-149 |
| ~~**E1** 自建 mini-indexer~~ | ~~5-10 探测号 24/7 爬群入库~~ | ~~2-3 周~~ | ~~🔴 重投入~~ | ❌ **2026-05-14 排除** — 服务器+IP 月费 $200-500 |
| **E2** | 行业 vertical 关键词包（博彩/外汇/加密/美容/教育 5 大行业 500+ 关键词）| 1-2 天 | 🟡 卖点 | ✅ 候选 (0 月费) |

#### vmfix30+ 锁定候选（**纯工程，无月费投入**）

经 2026-05-14 决策，**所有付费方案排除**。剩两条：

| # | 方案 | 工时 | 何时做 |
|---|---|---|---|
| **C5 live peek (A 方案)** | 实时拉群最近消息预览。新 task type `group_peek` + 前端任务状态轮询 | 3h | 用户「预览」按钮使用频次高时做 |
| **E2 行业关键词包** | 5 行业 × 100 词，新表 + onModuleInit seed + 前端「应用行业包」下拉 | 1-2 天 | 看租户行业集中度 — 80% 同行业立刻做 |

**触发条件**：vmfix29 上测试机/真租户跑 1-2 周后再决定。

### vmfix27 锁定候选（≈5 天工时）

**必修 (key feature drop)：**

1. **A1** messages.searchGlobal — **0.5 天**
2. **A2** 邀请链接收割 — **1 天**
3. **A3** AI 关键词扩展 — **1 天** ← **直接解 "健康 马来西亚 0 命中" 这类用户痛点**
4. **C3** 失败智能 fallback + UX 文案 — **1 天**
5. **C1** 关键词智能建议 — **0.5 天**
6. **B1** quality 算法 4 维度 — **1 天**
7. **D1** 24h cache — **0.5 天**

vmfix27 ship 后用户体验：输 1 个关键词 → 自动 AI 展开 5-10 变体 → searchGlobal + contacts.Search 并行 → 邀请链接挖洞 → 4 维度 quality 排序，**命中率从 30% 升到 85%+**，且每次搜不再重复消耗 TG API。
...
A5/A6/A7（滚雪球）/ A8（tgstat 商业决策）/ D2-D4 / E1-E2
